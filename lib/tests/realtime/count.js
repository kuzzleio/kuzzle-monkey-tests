  ...
  run () {
    ...
    return request.send()
      .then(response => {
        ...
        expected = 0;
        for (let sub of this._suite.world.subscriptionsByRoom.get(subscription.roomId)) {
          expected += sub.clients.size;
        }

        possible = 0;
        for (let sub of this._suite.world.subscriptionsUnconfirmed) {
