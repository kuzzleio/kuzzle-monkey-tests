...
class Client {
  ...
  _hasJustSubscribed (room, since = this._suite.config.subscriptions.historyCheckMaxDelay) {
  ...
  _hasJustUnsubscribed (room, since = this._suite.config.subscriptions.historyCheckMaxDelay) {
    let i = this.history.length;
    while (i--) {
      const request = this.history.get(i);
