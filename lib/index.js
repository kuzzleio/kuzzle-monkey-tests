...
class Suite {
  ...
  init () {
    ...
    if (!this.stopped) {
      this.log('info', util.inspect(this.world.stats, {depth: 5}));
      this.log('info', `duration: ${Date.now() - this.world.stats.started}ms`);
      this.log('info', 'rpm: ' + this.world.stats.requests.total / (Date.now() - this.world.stats.started) * 60 * 1000);
      this.log('info', 'SEED: ' + this.seed);
    }
    this.stopped = true;
  ...
  trace (whois, message) {
    return this.log('verbose', `${Date.now()}:${this.world.padRight(whois, 24)} ${message}`);
  }
